...
The first public hardware-validated image is:

```text
ghcr.io/david-lzy/vllm-gfx906:v0.28.0-gfx906.1
```

Patch releases increment the final gfx906 revision. A floating `latest` tag is
not published. Images must record the exact source commit and immutable image
digest, and the public release Dockerfile must not inherit a local Phase image.
