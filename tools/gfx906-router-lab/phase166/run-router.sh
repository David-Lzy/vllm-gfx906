...
stops, edits, or replaces the production Router.
EOF
}
...
case "$policy" in
  round_robin|global_fifo) ;;
  *)
    echo "unsupported Phase 166 policy: $policy" >&2
    exit 2
    ;;
esac
...
if [[ "$policy" == global_fifo ]]; then
  args+=(
    --global-fifo-worker-capacity 8
    --global-fifo-queue-size 96
    --global-fifo-queue-bytes 1073741824
    --global-fifo-queue-timeout-secs 43200
  )
fi
