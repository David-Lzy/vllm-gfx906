...
# Compare standard and copy-on-write packed-INT8 Qwen3.8 on a TP2x2 Router.
set -euo pipefail
...
readonly PHASE_SLUG=phase-152-qwen38-packed-int8-tp2x2-router
...
readonly STANDARD_MODEL_DIR=${STANDARD_MODEL_DIR:-$BUILD_ROOT/phase-102-qwen38-tp4-roofline-profile/model}
readonly PACKED_MODEL_DIR=${PACKED_MODEL_DIR:-$BUILD_ROOT/phase-82-qwen38-gdn-state-and-embedding-capacity/models/qwen38-embed-lmhead-int8}
...
capture_preflight() {
    ...
    require_file "$PACKED_MODEL_DIR/model-00002-of-00005.safetensors"
...
run_worker() {
    local name=$1 devices=$2 network=$3 alias=$4 model_dir=$5 cache_dir=$6 needs_source=$7
    local -a source_mount=()
    mkdir -p "$cache_dir/triton-cache"
    if [[ "$needs_source" == "1" ]]; then
        source_mount=(-v "$STANDARD_MODEL_DIR:/source:ro")
    fi
    docker run -d --name "$name" --network "$network" --network-alias "$alias" --ipc host --shm-size 64g \
        ...
        -v "$model_dir:/model:ro" "${source_mount[@]}" \
...
run_variant() {
    local key=$1 model_dir=$2 port=$3 needs_source=$4
    local base="phase152-${key}-tp2x2" directory="$RESULT_ROOT/$key/tp2x2-router"
    local network="${base}-net" worker0="${base}-worker0" worker1="${base}-worker1" router="${base}-router"
    local count0 count1
    SERVED_MODEL="qwen38-${key}-phase152"
    export SERVED_MODEL
    mkdir -p "$directory"
    docker network create "$network" >"$directory/network-id.txt"
    ACTIVE_NETWORKS+=("$network")
    run_worker "$worker0" 0,1 "$network" worker0 "$model_dir" "$PHASE_ROOT/cache/$key/worker0" "$needs_source"
    run_worker "$worker1" 2,3 "$network" worker1 "$model_dir" "$PHASE_ROOT/cache/$key/worker1" "$needs_source"
    ...
    wait_host_health "$port" "$router"
    docker run --rm --network host -v "$SCRIPT_DIR:/tools:ro" -v "$FIXTURE_DIR:/fixture:ro" \
        -v "$directory:/results" \
        --entrypoint /opt/vllm-venv/bin/python "$IMAGE" \
        /tools/benchmark_qwen38_packed_tp2x2.py \
        --endpoint "http://127.0.0.1:${port}/v1" --model "$SERVED_MODEL" --fixture "/fixture/$FIXTURE_NAME" \
        --asset-dir /results/assets --output /results/result.json --rounds 3 --timeout 900 \
        >"$directory/benchmark-summary.json"
    ...
    jq -n --argjson worker0 "$count0" --argjson worker1 "$count1" \
        '{worker0_chat_200:$worker0,worker1_chat_200:$worker1,difference:(($worker0-$worker1)|if . < 0 then -. else . end)}' \
        >"$directory/worker-distribution.json"
...
run_variant standard "$STANDARD_MODEL_DIR" 18162 0
run_variant packed-int8 "$PACKED_MODEL_DIR" 18163 1
restore_production
trap - EXIT
df -h /mnt/disk1 /mnt/disk2 / >"$RESULT_ROOT/disk-after.txt"
if [[ -d "$PHASE_ROOT/cache" ]]; then
    find "$PHASE_ROOT/cache" -depth -delete
fi
printf '%s\n' "$RESULT_ROOT"
