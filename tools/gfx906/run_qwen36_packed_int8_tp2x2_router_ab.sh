...
# Compare standard and copy-on-write packed-INT8 Qwen3.6 on a TP2x2 Router.
set -euo pipefail
...
readonly PHASE_SLUG=phase-150-qwen36-packed-int8-tp2x2-router
...
run_variant() {
    ...
    local base="phase150-${key}-tp2x2" directory="$RESULT_ROOT/$key/tp2x2-router"
    local network="${base}-net" worker0="${base}-worker0" worker1="${base}-worker1" router="${base}-router"
    local count0 count1
    SERVED_MODEL="qwen36-${key}-phase150"
    export SERVED_MODEL
    mkdir -p "$directory"
    ...
    docker run --rm --network host -v "$SCRIPT_DIR:/tools:ro" -v "$FIXTURE_DIR:/fixture:ro" \
        ...
        /tools/benchmark_qwen36_packed_tp2x2.py \
...
run_variant standard "$STANDARD_MODEL_DIR" 18150 0
run_variant packed-int8 "$PACKED_MODEL_DIR" 18151 1
restore_production
trap - EXIT
df -h /mnt/disk1 /mnt/disk2 / >"$RESULT_ROOT/disk-after.txt"
rm -rf "$PHASE_ROOT/cache"
printf '%s\n' "$RESULT_ROOT"
